...
def test_compute_scores():    
    ...
    component_results = compute_component_scores(
            validated_smilies, scoring_function, cache,
            invalid_mask & duplicate_mask
    )

    np.testing.assert_almost_equal(component_results.scores[0],
            np.array([180.159, 381.379, 206.285, 303.358, 44.097, 0.0,
                      311.363, 194.194, 303.358, 0.0, 0.0, 313.788,
                      477.603, 206.285]))


def test_compute_scores_duplicates_in_cache():
    DUPLICATES = ['CC(=O)Oc1ccccc1C(=O)O', 'Cn1c(=O)c2c(ncn2C)n(C)c1=O',
                  'CC(C)Cc1ccc(C(C)C(=O)O)cc1']
